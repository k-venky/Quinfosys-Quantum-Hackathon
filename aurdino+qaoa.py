...
import serial # ARDUINO INTEGRATION: for USB cable communication
...
VEHICLE_SPEED = 2.5   # Reduced speed for higher precision (prevents skipping destinations)
VEHICLE_SIZE = 2.0    # Smaller vehicle icon size
OBSTACLE_WARNING_DISTANCE = 3.0  # Distance before obstacle to show warning

# Hardware sync: only declare arrival / send hard stop when the simulation is
# very close to the final waypoint (helps prevent early 'X' on slower robots).
DESTINATION_REACH_EPS = 1.0              # Grid units (xy) tolerance
ARDUINO_STOP_CONFIRM_FRAMES = 5         # Require N consecutive frames close

# Auto reroute stability: prevent repeatedly re-applying alternate routes on the
# same obstacle event.
AUTO_REROUTE_COOLDOWN_FRAMES = 80

# Motor turn stability: avoid spamming L/R when direction jitters.
TURN_DP_THRESHOLD = 0.20                 # cosine(angle) threshold for turns
TURN_COMMAND_LOCK_FRAMES = 4            # keep sending same L/R for N frames
DENSIFY_STEP = 0.5                       # smaller step => smoother dense_path directions
...
# ============================================================================
# ARDUINO USB SERIAL CONFIGURATION
# ============================================================================
ARDUINO_PORT = "COM6"           # <--- UPDATE THIS with your Arduino COM Port
ARDUINO_BAUD = 9600
ARDUINO_SER = None 

def init_arduino_serial():
    """Initialize Arduino serial on the configured USB port."""
    global ARDUINO_SER
    print(f"[USB] Connecting to Arduino at {ARDUINO_PORT}...")
    try:
        # Create a serial object with DTR/RTS control to trigger reset
        ser = serial.Serial()
        ser.port = ARDUINO_PORT
        ser.baudrate = ARDUINO_BAUD
        ser.timeout = 0.1
        ser.open()
        
        # Trigger Arduino Reset
        ser.dtr = False
        time.sleep(0.1)
        ser.reset_input_buffer()
        ser.dtr = True
        
        # Give Arduino time to reset and send "ARDUINO_READY"
        time.sleep(2.0)
        
        ARDUINO_SER = ser
        print(f"[USB] Successfully connected on {ARDUINO_PORT}!")
    except Exception as e:
        ARDUINO_SER = None
        print(f"[USB] Connection failed: {e}")
        print("Note: Ensure your Arduino is plugged in and the correct COM port is set.")

def arduino_send(cmd):
    """
    Send a single-character command via USB Serial.
    """
    global ARDUINO_SER
    if not ARDUINO_SER:
        return
    try:
        if ARDUINO_SER.is_open:
            ARDUINO_SER.write(str(cmd).encode("utf-8"))
...
def read_arduino_feedback():
    """
    Non-blocking read of Robot feedback (like EVENT:OBSTACLE).
    """
    global ARDUINO_SER
    if not ARDUINO_SER:
        return None
    try:
        if ARDUINO_SER.is_open and ARDUINO_SER.in_waiting > 0:
            line = ARDUINO_SER.readline().decode("utf-8", errors="ignore").strip()
            if line:
                print(f"[ROBOT] {line}")
                if "OBSTACLE" in line.upper():
                    return "OBSTACLE"
                return line
    except Exception as e:
        pass
...
class VehicleState:
    ...
    def __init__(self, name, route_nodes, color, alt_route=None):
        ...
        self.arrival_confirm_frames = 0  # consecutive frames within EPS of destination
        # When a sharp turn is detected, lock the command to avoid jitter.
        self.turn_lock_frames = 0
        self.turn_lock_cmd = None
...
class RouteSimulation:
    def __init__(
        ...
        # Lock auto-reroute to a single alternate route until the vehicle
        # reaches its destination (prevents "shifting" between alternates).
        self.auto_reroute_locked = False
        self.auto_reroute_locked_route = None
        ...
        # Auto reroute cooldown (used by the predictive / visual obstacle path)
        self.last_auto_reroute_frame = -AUTO_REROUTE_COOLDOWN_FRAMES
    ...
    def _build_vehicle_states(self):
        ...
        v.dense_path = self._densify_path(v.path, step=DENSIFY_STEP)
    ...
    def _direction_to_arduino_cmd(self, prev_dir, new_dir):
        ...
        # If angle change is large (cosine below threshold), signal a turn.
        # This prevents jitter on minor corrections.
        if dp < TURN_DP_THRESHOLD:
    ...
    def apply_route_update(self, vehicle, new_route_nodes):
        ...
        # Find the closest *sensible* node in new route.
        # Important: many routes start at depot node `0`. If we blindly pick the
        # nearest node, the depot can become the "closest" and the vehicle
        # will re-route back to the start.
        current_pos = np.array(vehicle.pos, dtype=float).copy()
        depot_pos = self.points[0]
        dist_to_depot = float(np.linalg.norm(current_pos - depot_pos))
        move_dir = getattr(vehicle, "move_dir", None)

        best_idx = None
        min_dist = float('inf')

        def _candidate_ok(node_idx: int) -> bool:
            # Avoid re-selecting depot unless we're already near it.
            if node_idx == 0:
                if self.end_point is not None:
                    # When we have a separate destination, don't re-route back
                    # to the start/depot unless we're truly at/near it.
                    if dist_to_depot > DESTINATION_REACH_EPS * 0.8:
                        return False
                else:
                    # Round-trip mode: allow depot selection only when we're near.
                    if dist_to_depot > DESTINATION_REACH_EPS * 2.0:
                        return False

            # Prefer nodes "ahead" in the direction of travel if we have it.
            if move_dir is not None:
                node_pos = self.points[node_idx]
                vec = node_pos - current_pos
                if np.dot(vec, move_dir) <= 0:
                    # Still allow destination so we can end correctly.
                    if self.end_point is not None and node_idx == self.destination_idx:
                        return True
                    return False
            return True

        # First pass: try filtered candidates (avoid depot / behind nodes)
        for i, node in enumerate(new_route):
            if not _candidate_ok(node):
                continue
            node_pos = self.points[node]
            dist = float(np.linalg.norm(node_pos - current_pos))
            if dist < min_dist:
                min_dist = dist
                best_idx = i

        # Fallback: if everything got filtered out, revert to closest-node logic
        if best_idx is None:
            best_idx = 0
            min_dist = float('inf')
            for i, node in enumerate(new_route):
        ...
        # Safety: when destination mode is enabled, never intentionally start
        # the reroute by going back to depot `0`.
        if self.end_point is not None and remaining_nodes and remaining_nodes[0] == 0:
        ...
        vehicle.dense_path = self._densify_path(vehicle.path, step=DENSIFY_STEP)
    ...
    def animate(self, frame):
        ...
        # If we've already committed to one alternate route, don't keep
        # switching between alternates mid-run.
        if (not self.obstacle_detected) and self.vehicles and not self.auto_reroute_locked:
            v = self.vehicles[0]
                    ...
                    if lookahead_dist >= OBSTACLE_WARNING_DISTANCE * 0.3 and frame >= self.obstacle_frame:
                        ...
                        if best_edge_idx < len(v.route_nodes) - 1:
                            ...
                            if self.alt_routes_ui:
                                ...
                                self.active_alternate_cost = float(best_cost)
                                self.switched = True
                                # Cooldown to prevent repeated re-applying on the same
                                # obstacle event (which can cause looping back to depot).
                                if frame - self.last_auto_reroute_frame >= AUTO_REROUTE_COOLDOWN_FRAMES:
                                    self.last_auto_reroute_frame = frame
                                    self.apply_route_update(v, best_route)
                                    self.auto_reroute_locked = True
                                    self.auto_reroute_locked_route = best_route
                                else:
                                    print("[QAOA] Auto-reroute cooldown active; skipping duplicate reroute.")
        ...
        # Base step size for movement.
        # Important: if this is too fast, we can reach the final waypoint in the
        # simulator and send 'X' before the physical robot arrives.
        base_step = VEHICLE_SPEED * 1.0
        ...
        for v in self.vehicles:
            ...
            if v.segment_index >= len(v.dense_path) - 1:
                # Confirm arrival using distance to the final waypoint.
                # This prevents premature hard-stop when the simulator outruns hardware.
                final_pt = np.array(v.dense_path[-1]) if v.dense_path else None
                pos_pt = np.array(v.pos) if v.pos is not None else None
                dist_to_final = float('inf')
                if final_pt is not None and pos_pt is not None:
                    dist_to_final = np.linalg.norm(pos_pt - final_pt)

                if dist_to_final <= DESTINATION_REACH_EPS:
                    v.arrival_confirm_frames += 1
                    # While we're "close enough" but not yet confirmed, keep the robot
                    # moving forward so we don't accidentally stall before the final
                    # hard-stop command ('X').
                    if v.arrival_confirm_frames < ARDUINO_STOP_CONFIRM_FRAMES:
                        if getattr(v, "last_sent_cmd", None) != "F" and not self.obstacle_detected:
                            arduino_send("F")
                            v.last_sent_cmd = "F"
                else:
                    # If we somehow advanced to the end but aren't close enough,
                    # step back so the movement logic can continue.
                    v.arrival_confirm_frames = 0
                    if len(v.dense_path) >= 2:
                        v.segment_index = max(0, len(v.dense_path) - 2)
                    # Fall through to normal movement update below.
                    if v.segment_index < len(v.dense_path) - 1:
                        pass

                if v.segment_index >= len(v.dense_path) - 1:
                    if (not v.done) and v.arrival_confirm_frames >= ARDUINO_STOP_CONFIRM_FRAMES:
                        v.done = True
                        arduino_send('X')  # IMMEDIATELY STOP HARDWARE ON VERIFIED ARRIVAL
                        print(f"\n[LOG] {v.name} has arrived at the destination!")
                    # Keep looping; visuals will render at the current position.
                    continue
            ...
            if dist > 1e-6:
                ...
                # If we're currently locking onto a sharp turn, keep sending
                # the same turn command for a few frames to avoid jitter.
                if getattr(v, "turn_lock_frames", 0) > 0 and v.turn_lock_cmd in ("L", "R"):
                    cmd = v.turn_lock_cmd
                    v.turn_lock_frames -= 1
                else:
                    cmd = self._direction_to_arduino_cmd(prev_dir, new_dir)

                    # When we detect a sharp turn, lock it briefly.
                    if cmd in ("L", "R"):
                        v.turn_lock_cmd = cmd
                        v.turn_lock_frames = TURN_COMMAND_LOCK_FRAMES
                ...
                # dist is tiny/zero; either keep the turn lock alive or fall back to forward.
                if getattr(v, "turn_lock_frames", 0) > 0 and v.turn_lock_cmd in ("L", "R"):
                    cmd = v.turn_lock_cmd
                    v.turn_lock_frames -= 1
                    last_cmd = getattr(v, "last_sent_cmd", None)
        ...
        if all(v.done for v in self.vehicles):
            ...
            # Unlock auto reroute once mission is done.
            self.auto_reroute_locked = False
            self.auto_reroute_locked_route = None
...
def main():
    ...
    if ARDUINO_SER:
        ...
        while True:
            ...
            feedback = read_arduino_feedback()
            normalized = feedback.strip().upper() if isinstance(feedback, str) else ""
            if normalized in ("READY", "ARDUINO_READY"):
